...
export const REQUEST_STATUSES = ["pending", "approved", "denied", "waitlisted"] as const;
...
Rate limits are documented as target behavior, but are not enforced by the current API implementation yet.

- Planned: SSH submissions limited per fingerprint/IP window
- Planned: Admin API request throttling
